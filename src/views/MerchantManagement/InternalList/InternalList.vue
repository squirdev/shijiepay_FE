...
import { getInternalListStatisticsApi, getInternalListApi } from '@/api/merchantmanagement/index'
import { useTable } from '@/hooks/web/useTable'
import { ref, unref, reactive, onMounted } from 'vue'
import { CrudSchema, useCrudSchemas } from '@/hooks/web/useCrudSchemas'
import { Breadcrumb } from '@/components/Breadcrumb'
import { ElMessage, ElMessageBox, ElRow, ElCol, ElLoading } from 'element-plus'
...
const { tableRegister, tableState, tableMethods } = useTable({
  fetchDataApi: async () => {
    ...
    const res = await getInternalListApi({
...
const getStatistics = async () => {
  ...
  try {
    const res = await getInternalListStatisticsApi(searchParams.value)
    totalData.totalInternalAmount = res.data.actual_amount_total
    totalData.totalInternalFee = res.data.repay_amount_total
...
<template>
  <div>
    <ContentWrap class="search-section">
      <div class="log-search">
        ...
        <Search
          ...
          :show-order-statistics="hasPermi('rechargeMoneyTotal')"
    ...
    >
      <ElRow>
        <ElCol :span="13" :tag="'span'" style="text-align: right; padding: 10px"> 总金额: </ElCol>
        <ElCol :span="11" :tag="'span'" style="color: black; padding: 10px">
          {{ totalData.totalInternalAmount }}
        </ElCol>
      </ElRow>
      <ElRow>
        <ElCol :span="13" :tag="'span'" style="text-align: right; padding: 10px"> 总手续费: </ElCol>
        <ElCol :span="11" :tag="'span'" style="color: black; padding: 10px">
