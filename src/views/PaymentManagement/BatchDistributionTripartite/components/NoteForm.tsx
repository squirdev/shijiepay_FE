...
const NoteForm = defineComponent({
  ...
  setup(props, { expose }) {
    ...
    const modelValue = ref({ new_note: '' })

    onMounted(() => {
      modelValue.value = { new_note: props.row.new_note ?? '' }
    })
    ...
    return () => (
      <ElForm model={modelValue} ref={baseFormRef} labelWidth="100px" labelPosition="right">
        ...
        >
          <ElInput
            placeholder="请输入"
            modelValue={modelValue.value.new_note}
            onUpdate:modelValue={(v) => (modelValue.value['new_note'] = v)}
          />
        </ElFormItem>
      </ElForm>
    )
