...
import { getTunnelRateApi, tunnelRateApi } from '@/api/channelmanagement'
...
export const useTunnelTable = (searchParams: Ref<SearchParams>) => {
  ...
  const { tableRegister, tableState, tableMethods } = useTable({
    fetchDataApi: async () => {
      ...
      try {
        const res = await getTunnelRateApi({
          pageIndex: unref(currentPage),
          pageSize: unref(pageSize),
          ...unref(searchParams)
        })
        tunnelDatas.value = res.data?.tunnel_datas?.map((item) => ({
          label: item.name,
          value: item.uuid,
    ...
    fetchDelApi: async () => {
      const res = await tunnelRateApi({ data_uuid: unref(ids)[0], action: 'del' })
      return !!res.success
    }
  })
  ...
  const handleStatusChange = async (row: TunnelType) => {
    ...
    try {
      ...
      const res = await tunnelRateApi(row)
      if (res?.success) {
