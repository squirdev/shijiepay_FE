  ...
  setup(props, { emit }) {
    ...
    const fetchInfo = async (uuid: string) => {
      ...
      try {
        ...
        const countryTunnelDatas = Object.values(retData.data.countryTunnelDatas).filter(
          (l) => l.tunnel_uuid
        )
    ...
    const saveDialog = async () => {
      ...
      const newValues = {
        action: 'payment_test_order',
        data_uuid: props.row.uuid,
        merchant_id: modelValue.value.merchant_id,
        amount: modelValue.value.amount,
        order_id: modelValue.value.order_id,
        tunnel_uuid: modelValue.value.tunnel_uuid
      }
          ...
          ElMessageBox.alert(
            <div>
              <div class="flex h-42px items-center text-14px">
                <span class="w-100px text-right mr-20px color-[#636363]">支付url:</span>
                <ElTooltip placement="top" content={res.data.payUrl ?? ''}>
                  <span class="color-[#0e0e0e] break-all flex-1 line-clamp-2">
                    {res.data.payUrl ?? ''}
                  </span>
                </ElTooltip>
              </div>
              <div class="flex h-42px items-center text-14px">
                ...
                <span class="color-[#0e0e0e]">{res.data.systemOrderId ?? ''}</span>
              </div>
              <div class="flex h-42px items-center text-14px">
      ...
      >
        <div class="pl-4px">
          <ElForm model={modelValue} ref={baseFormRef} labelWidth="100px" labelPosition="right">
            ...
            >
              <ElSelect
                v-model={modelValue.value.tunnel_uuid}
                placeholder={t('common.selectText')}
                class="w-1/1"
              >
                {testData.value?.countryTunnelDatas?.map((item) => (
                  <ElOption
                    key={item.tunnel_uuid}
                    label={item.tunnel_name}
                    value={item.tunnel_uuid}
                  />
                ))}
              </ElSelect>
            </ElFormItem>
