...
const TIME_AGO_MESSAGE_MAP: {
  ...
  vi: {
    ...
    future: (n) => (n.match(/\d/) ? `trong ${n}` : n),
    month: (n, past) => (n === 1 ? (past ? 'tháng trước' : 'tháng sau') : `${n} tháng`),
    year: (n, past) => (n === 1 ? (past ? 'năm ngoái' : 'năm sau') : `${n} năm`),
    day: (n, past) => (n === 1 ? (past ? 'hôm qua' : 'ngày mai') : `${n} ngày`),
    week: (n, past) => (n === 1 ? (past ? 'tuần trước' : 'tuần sau') : `${n} tuần`),
    hour: (n) => `${n} giờ`,
    minute: (n) => `${n} phút`,
    second: (n) => `${n} giây`
  }
