...
import { calculateColumnWidth } from '@/components/Table/src/helper'
...
const filterTableSchema = (crudSchema: CrudSchema[]): TableColumn[] => {
  const tableColumns = treeMap<CrudSchema>(crudSchema, {
    conversion: (schema: CrudSchema) => {
      if (!schema?.table?.hidden) {
        // 如果提供了 min-width，则使用提供的值；否则根据 label 自动计算
        const minWidth = schema?.['min-width']
        const autoWidth = calculateColumnWidth(schema.label, minWidth)
  ...
  const resultTableColumns = filter<TableColumn>(tableColumns as TableColumn[], (data) => {
