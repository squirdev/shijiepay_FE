...
import { setIndex, calculateColumnWidth } from './helper'
  ...
  setup(props, { attrs, emit, slots, expose }) {
    ...
    const renderTreeTableColumn = (columnsChildren: TableColumn[]) => {
      ...
      return columnsChildren.map((v) => {
        ...
        // 自动计算列宽：如果没有设置 minWidth，则根据 label 自动计算
        if (!props.minWidth && !props['min-width']) {
          const autoWidth = calculateColumnWidth(v.label)
          if (autoWidth !== undefined) {
            props['min-width'] = autoWidth
          }
        }
            ...
            <>
              ...
              <ElTable
                ...
                style={{
                  ...
                  '--el-table-row-hover-bg-color': '#f6f8fb',
                  '--el-table-text-color': '#606279',
                  '--el-table-border-color': '#dfe6ec'
                }}
              >
                {{
